namespace COMP003A.LectureActivity7
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            // arrays store a collection of data of the same type (int, string, objects, etc.)
            // array1 decleration and setting values
            int[] array1 = new int[5]; // declares a single-dimensiona array of 5 integers without values
            // you can set value to an index using the examples below:
             ...
             * Also, you will get a runtime error of OutOfBoundsException when accessing an index that does not exist
             */
            ...
            Console.WriteLine("\nstring Traversal");
            // strings are a special object consisting of an array of characters
            string message = "hello";
            ArrayTraversal(message);

            SectionSeparator("Lists");
            // like arrays, lists store a collection of data with the same type (EX: int, string, objects, etc.)
            // unlike arrays, lists are dynamic and can increase/decrease in size.
            List<char> alphabet= new List<char>(); // declares an empty integer list
        ...
        {
            // you can use any looping statements to traverse through arrays
            for (int i = 0; i < array.Length;i++)
            {
                Console.WriteLine($"Array at index {i} : {array[i]}");
            }
